...
# - A file called metadata.yaml (in YAML).  This is the final ultimate source for all the reasons mentioned above, easy to edit, and powerful.
...
#   - The form determines the schema, which can be changed, but its pretty regular.
...
def summary_table(race_records):
    "Return a summary table the races."
    rows = []
    for race in race_records:
        row = {k:race.get(k, '') for k in "date title file source".split()}
        rows.append(row)
...
def check_columns_changed(df, long_colname_map):
    ...
    if len(new_colnames) == 0 and len(missing_colnames) == 0:
        ...
        G.logger.warn(f"Uh Oh. New cols {new_colnames}. Missing cols {missing_colnames}.")
        return False
            ...
            if date not in dates:
                ...
                if source == 'byhand':
                    if metadata.timestamp < record['timestamp']:
                        G.logger.warn(f"Duplicate record. GSheet row is newer than byhand metadata: {record['timestamp']}.")
